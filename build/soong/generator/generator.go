...
func init() {
	android.RegisterModuleType("ryu_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = ryuExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = ryuExpandVariables(ctx, dep_file)
	...
	cmd := ryuExpandVariables(ctx, String(g.properties.Cmd))
