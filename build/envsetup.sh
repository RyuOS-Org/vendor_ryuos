function __print_ryuos_functions_help() {
cat <<EOF
Additional RyuOS functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- ryuosgerrit:   A Git wrapper that fetches/pushes patch from/to RyuOS Gerrit Review.
- ryuosrebase:   Rebase a Gerrit change and push it again.
- ryuosremote:   Add git remote for RyuOS Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for RyuOS Github.
...
function breakfast()
{
    target=$1
    local variant=$2
    source ${ANDROID_BUILD_TOP}/vendor/ryuos/vars/aosp_target_release
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the RyuOS model name
            if [ -z "$variant" ]; then
                variant="user"
            fi
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/ryuos-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.ryuos.device | grep -q "$RYUOS_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $RYUOS_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function ryuosremote()
{
    ...
    git remote rm ryuos 2> /dev/null
    ...
    local RYUOS_USER=$(git config --get review.review.ryuos.org.username)
    if [ -z "$RYUOS_USER" ]
    then
        git remote add ryuos ssh://review.ryuos.org:29418/$PFX$PROJECT
    else
        git remote add RYUOS ssh://$RYUOS_USER@review.ryuos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'ryuos' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/RyuOS-Org/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.ryuos.device | grep -q "$RYUOS_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        ryuosremote
        git push ryuos HEAD:refs/heads/'$1'
    fi
    '
}

function ryuosgerrit() {
    ...
    local user=`git config --get review.review.ryuos.org.username`
...
function ryuosrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "RyuOS Gerrit Rebase Usage: "
        echo "      ryuosrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.ryuos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.ryuos.device | grep -q "$RYUOS_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/ryuos/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $RYUOS_FIXUP_COMMON_OUT ]; then
